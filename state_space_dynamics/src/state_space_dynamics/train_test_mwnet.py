...
def resume_main(dataset_dir: pathlib.Path,
                checkpoint: str,
                batch_size: int,
                epochs: int,
                user: str,
    ...
    dataset_dir = fetch_udnn_dataset(dataset_dir)
    train_dataset = TorchMetaDynamicsDataset(dataset_dir, transform=transform)
    train_dataset_repeat = repeat_dataset(train_dataset, repeat)
    train_loader = DataLoader(train_dataset_repeat,
                              batch_size=batch_size,
                              shuffle=True,
                              collate_fn=my_collate,
    ...
    wb_logger = WandbLogger(project=project, name=checkpoint, id=checkpoint, log_model='all', entity=user)
    ckpt_cb = pl.callbacks.ModelCheckpoint(monitor="val_loss", save_top_k=1, save_last=True, filename='{epoch:02d}')
    trainer = pl.Trainer(gpus=1,
                         logger=wb_logger,
                         enable_model_summary=False,
    ...
    trainer.fit(model, train_loader, val_dataloaders=train_loader)
    wandb.finish()
    # eval_main(dataset_dir, checkpoint, mode='test', user=user, batch_size=batch_size)
    return checkpoint
