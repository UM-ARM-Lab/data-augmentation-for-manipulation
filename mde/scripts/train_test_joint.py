...
from mde import train_test_joint
from moonshine.magic import wandb_lightning_magic

node_name = f"train_test_joint_{int(time())}"
...
def main():
    def _train_main(args):
        ...
        train_test_joint.train_main(**vars(args))

    def _eval_main(args):
        train_test_joint.eval_main(**vars(args))

    def _viz_main(args):
        train_test_joint.viz_main(**vars(args))
