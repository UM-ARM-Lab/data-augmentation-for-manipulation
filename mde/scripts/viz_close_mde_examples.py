...
def main():
    ...
    val_dataset = TorchMDEDataset(fetch_mde_dataset(args.dataset), mode='val')
    ...
    val_actions_list = []
    for val_traj in val_dataset:
        if val_traj['example_idx'] in args.example_indices:
            print(val_traj['example_idx'])
            get_actions(val_actions_list, val_dataset, val_traj)

    train_actions_list = get_actions_list(train_dataset)

    viz_data = compute_viz_data(val_actions_list, train_actions_list)
...
def compute_viz_data(ref_actions_list, train_actions_list):
    ...
    data = []
    # TODO: fix this to find the closest example in train to each example in ref
...
def get_actions_list(dataset):
    ...
    for traj in tqdm(dataset):
        get_actions(actions_list, dataset, traj)
    return actions_list


def get_actions(actions_list, dataset, traj):
