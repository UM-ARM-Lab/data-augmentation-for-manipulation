...
def load_dynamics_dataset(dataset_dir: List[pathlib.Path], **kwargs):
    d_for_checking_type = dataset_dir[0]
    for e in d_for_checking_type.iterdir():
        if e.is_file():
            if 'tfrecord' in e.as_posix():
                return DynamicsDatasetLoader(dataset_dir, **kwargs)
            elif 'pkl' in e.as_posix():
                return NewDynamicsDatasetLoader(dataset_dir, **kwargs)
        elif e.is_dir():
...
def load_classifier_dataset(dataset_dir: List[pathlib.Path], **kwargs):
    d_for_checking_type = dataset_dir[0]
    for e in d_for_checking_type.iterdir():
        if e.is_file():
            if 'tfrecord' in e.as_posix():
                return ClassifierDatasetLoader(dataset_dir, **kwargs)
            elif 'pkl' in e.as_posix():
                return NewClassifierDatasetLoader(dataset_dir, **kwargs)
        elif e.is_dir():
            for sub_e in e.iterdir():
                if sub_e.is_file():
                    if 'tfrecord' in sub_e.as_posix():
