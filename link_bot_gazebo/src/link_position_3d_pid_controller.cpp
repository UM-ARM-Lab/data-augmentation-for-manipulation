...
LinkPosition3dPIDController::LinkPosition3dPIDController(char const *plugin_name, physics::LinkPtr link,
                                                         bool position_only, double const kp_pos, double const kp_vel,
                                                         double const kd_vel, double const max_force,
                                                         double const max_vel, bool const grav_comp)
    : BaseLinkPositionController(plugin_name, link, "pid", position_only, false),
      kP_pos_(kp_pos),
      kP_vel_(kp_vel),
      kD_vel_(kd_vel),
...
void LinkPosition3dPIDController::Update(ignition::math::Pose3d const &setpoint) {
  ...
  ROS_DEBUG_STREAM_NAMED(plugin_name_ + ".params", "kPs: " << kP_pos_ << " " << kP_vel_);
  ROS_DEBUG_STREAM_NAMED(plugin_name_ + ".vel", "target vel: " << target_vel << " max vel " << max_vel_);
  ROS_DEBUG_STREAM_NAMED(plugin_name_ + ".error", "pos error " << pos_error_ << " velocity error " << vel_error);
  ROS_DEBUG_STREAM_NAMED(plugin_name_ + ".force", "force " << force.X() << " " << force.Y() << " " << force.Z());
