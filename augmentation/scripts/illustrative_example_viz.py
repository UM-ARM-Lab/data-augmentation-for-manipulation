...
def viz_traj(dataset_dir, checkpoint):
    ...
    b = 0
    obj_pred_pos_xy = pred_pos[0, :, 1].cpu().detach().numpy()
    obj_gt_pos_xy = gt_pos[0, :, 1].cpu().detach().numpy()
    robot_gt_pos_xy = gt_pos[0, :, 0].cpu().detach().numpy()
    z = np.expand_dims(np.ones_like(obj_gt_pos_xy[..., 0], np.float32), -1) * 0.14
    obj_pred_pos_xyz = np.concatenate((obj_pred_pos_xy, z), -1)
    obj_gt_pos_xyz = np.concatenate((obj_gt_pos_xy, z), -1)
    robot_gt_pos_xyz = np.concatenate((robot_gt_pos_xy, z), -1)
    s.plot_line_strip_rviz(obj_pred_pos_xyz, label='predicted', color='#333399', scale=0.005)
    s.plot_line_strip_rviz(obj_gt_pos_xyz, label='actual', color='#993333', scale=0.005)
    s.plot_line_strip_rviz(robot_gt_pos_xyz, label='robot', color='#993399', scale=0.005)
    ...
    s.plot_state_rviz(pred_state_t, label=f'predicted', color=(0, 0, 1, 1), no_robot=True)
