...
def make_robot_voxelgrid(inputs, local_origin_point, t, info: MakeVoxelgridInfo):
    names = inputs['joint_names']
    positions = inputs['joint_positions']
    robot_points = inputs['robot_points']
    ...
    points_robot_frame = batch_transform_robot_points(info.jacobian_follower,
                                                      names,
                                                      positions,
                                                      info.points_per_links,
                                                      info.points_link_frame,
                                                      info.link_names)
...
def make_voxelgrid_inputs_t(
    ...
    channel_idx = 1
    for (k, state_component_t) in enumerate(state_t.items()):
