...
def iterative_fine_tuning(training_dataset_dir: pathlib.Path,
    ...
    for i, (train_dataset_chunk, val_dataset_chunk) in enumerate(zip(train_dataset_chunks, val_dataset_chunks)):
        ...
        if results_i is None:
            ...
            results_i['seed'] = seed
            results_i['augmentation_type'] = has_keys(model_hparams_update, ['augmentation', 'type'], noop_val=None)
            results_i['training_dataset_dir'] = training_dataset_dir.as_posix()
            results_i['train_dataset_size'] = len(train_dataset_chunk)
            results_i['val_dataset_size'] = len(val_dataset_chunk)
        ...
        if new_trial_path is None:
            ...
            adaptive_batch_size = compute_batch_size_for_n_examples(total_examples=len(train_dataset_chunk),
                                                                    max_batch_size=32)
            print(f'{adaptive_batch_size=}')
